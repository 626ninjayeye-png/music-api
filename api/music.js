export default async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  res.setHeader('Content-Type', 'application/json; charset=utf-8');

  const name = req.query.name?.trim();
  if (!name) {
    return res.json({ code: 400, msg: '请输入歌曲名' });
  }

  // === 你的真实 cookie（已填好）===
  const cookie = 'MUSIC_U=';

  try {
    // 使用更稳定的 cloudsearch 接口（POST）
    const searchRes = await fetch('https://music.163.com/weapi/cloudsearch/get/web?csrf_token=', {
      method: 'POST',
      headers: {
        'Cookie': cookie,
        'Referer': 'https://music.163.com/',
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36',
        'Content-Type': 'application/x-www-form-urlencoded',
      },
      body: new URLSearchParams({
        s: name,
        type: '1',
        offset: '0',
        limit: '3',
        total: 'true'
      })
    });

    const searchData = await searchRes.json();
    const song = searchData?.result?.songs?.[0];

    if (!song) {
      return res.json({ code: 404, msg: '未找到歌曲' });
    }

    const songId = song.id;
    const title = song.name;
    const singer = song.ar?.map(a => a.name).join('/') || '未知歌手';
    const cover = song.al?.picUrl || `https://p1.music.126.net/UeTuwE7pvjBpypWLudqukA==/109951165000000.jpg`;
    const link = `https://music.163.com/#/song?id=${songId}`;
    const music_url = `https://music.163.com/song/media/outer/url?id=${songId}.mp3`;

    // 获取歌词
    const lyricRes = await fetch(
      `https://music.163.com/api/song/lyric?id=${songId}&lv=1&tv=1`,
      {
        headers: {
          'Cookie': cookie,
          'Referer': 'https://music.163.com/'
        }
      }
    );
    const lyricData = await lyricRes.json();
    let lyric = (lyricData?.lrc?.lyric || '')
      .replace(/\[\d{2}:\d{2}\.\d{2,3}\]/g, '')
      .split('\n')
      .filter(l => l.trim())
      .join('\n');

    res.json({
      code: 200,
      title,
      singer,
      cover,
      link,
      music_url,
      lyric: lyric || '暂无歌词'
    });

  } catch (e) {
    console.error(e);
    res.json({ code: 500, msg: '服务器错误' });
  }
}
